...
```sh
make docker-build docker-push IMG=<some-registry>/vault-restart:tag
```
...
```sh
make deploy IMG=<some-registry>/vault-restart:tag
```
...
```sh
make build-installer IMG=<some-registry>/vault-restart:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/vault-restart/<tag or branch>/dist/install.yaml
```
